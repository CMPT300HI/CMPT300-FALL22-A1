...
def main():
    ...
    '''
    if os.path.exists('githubid.txt'):
        getId = open('githubid.txt','r')
        getStudentId = getId.read().strip()
    else:
        getStudentId = "unknown_user"
    totalPoint = 0
    x = checkMakefile()
    if x:
        totalPoint = totalPoint + 10 #makefile is working
    else:
        return totalPoint
    prob1Point = 0
    prob2Point = 0
    ...
    if os.path.exists('output2.txt'):
        ...
        if abs(prob2Result - memUtilResult) < 6:
            prob2Point = prob2Point + 15
            totalPoint = totalPoint + prob2Point 
    makefilePoint = totalPoint - (prob1Point+prob2Point+prob3Point)
    if getStudentId == "unknown_user":
        totalPoint = totalPoint - 10
        gradePoint = {getStudentId : [totalPoint,makefilePoint,prob1Point,prob2Point,prob3Point]}
    else:
        gradePoint = {getStudentId : [totalPoint,makefilePoint,prob1Point,prob2Point,prob3Point]}
    writeToGradeFile = open('grade.txt','w')
    writeToGradeFile.write(str(gradePoint))
    return 0
